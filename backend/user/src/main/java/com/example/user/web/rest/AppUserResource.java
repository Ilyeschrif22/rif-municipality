...
import com.example.user.web.rest.errors.BadRequestAlertException;
...
/**
 * REST controller for managing {@link com.example.user.domain.AppUser}.
 */
@RestController
...
public class AppUserResource {
    ...
    /**
     * {@code GET  /app-users/cin/:cin} : get the appUser by cin.
     *
     * @param cin the cin of the appUserDTO to retrieve.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the appUserDTO, or with status {@code 404 (Not Found)}.
     */
    @GetMapping("/cin/{cin}")
    public ResponseEntity<AppUserDTO> getAppUserByCin(@PathVariable("cin") String cin) {
        LOG.debug("REST request to get AppUser by cin : {}", cin);
        Optional<AppUserDTO> appUserDTO = appUserService.findByCin(cin);
        return ResponseUtil.wrapOrNotFound(appUserDTO);
    }
