...
export const Request = () => {
  ...
  const requestList = useAppSelector(state => state.gateway.request.entities);
  const loading = useAppSelector(state => state.gateway.request.loading);
  const totalItems = useAppSelector(state => state.gateway.request.totalItems);
  ...
  return (
    <div>
      <h2 id="request-heading" data-cy="RequestHeading">
        <Translate contentKey="gatewayApp.request.home.title">Requests</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="gatewayApp.request.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/request/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="gatewayApp.request.home.createLabel">Create new Request</Translate>
          </Link>
        </div>
      </h2>
        ...
        {requestList && requestList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="gatewayApp.request.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('type')}>
                  <Translate contentKey="gatewayApp.request.type">Type</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('type')} />
                </th>
                ...
                <th className="hand" onClick={sort('resolvedDate')}>
                  ...
                  <FontAwesomeIcon icon={getSortIconByFieldName('resolvedDate')} />
                </th>
            ...
            <tbody>
              {requestList.map((request, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`gatewayApp.RequestStatus.${request.status}`} />
                  </td>
                  <td>{request.createdDate ? <TextFormat type="date" value={request.createdDate} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{request.resolvedDate ? <TextFormat type="date" value={request.resolvedDate} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{request.citizenId}</td>
                  <td>{request.municipalityId}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/request/${request.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="gatewayApp.request.home.notFound">No Requests found</Translate>
            </div>
          )
        )}
      </div>
