...
import com.example.request.web.rest.errors.BadRequestAlertException;
...
/**
 * REST controller for managing {@link com.example.request.domain.Request}.
 */
@RestController
...
public class RequestResource {
    ...
    public ResponseEntity<RequestDTO> createRequest(@Valid @RequestBody RequestDTO requestDTO) throws URISyntaxException {
        ...
        return ResponseEntity.created(new URI("/api/requests/" + requestDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, requestDTO.getId().toString()))
            .body(requestDTO);
    }
